...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingDto> getAllByUser(BookingRequestState state, Long userId, Integer from, Integer size) {
        if (Optional.ofNullable(userRepository.getReferenceById(userId).getName()).isEmpty()) {
            ...
            if (((from < 0) || size < 0) || ((from == 0) && (size == 0))) {
                ...
                bookingListResult = bookingRepository.findByBookerIdWithPagination(userId, size, from);
            }
    ...
    @Override
    public List<BookingDto> getAllByOwner(BookingRequestState state, Long userId, Integer from, Integer size) {
        ...
        if (Optional.ofNullable(userRepository.getReferenceById(userId).getName()).isEmpty()) {
            ...
            if (((from < 0) || size < 0) || ((from == 0) && size == 0)) {
                ...
                for (Item item : itemListResult) {
                    bookingListResult.addAll(bookingRepository.findByItemIdWithPagination(item.getId(), size, from));
                }
                System.out.println("check");
            }
            List<Booking> bookingAll = bookingRepository.findAll();
            System.out.println(bookingAll);
